...
def _engineer_features(X: np.ndarray) -> np.ndarray:
    ...
    emp = X[:, n + CATEGORICAL_FEATURES.index("employment"): n + CATEGORICAL_FEATURES.index("employment") + 1]
    chk = X[:, n + CATEGORICAL_FEATURES.index("checking_status"): n + CATEGORICAL_FEATURES.index("checking_status") + 1]
    sav = X[:, n + CATEGORICAL_FEATURES.index("savings_status"): n + CATEGORICAL_FEATURES.index("savings_status") + 1]
    return np.hstack([X,
