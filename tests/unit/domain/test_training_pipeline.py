...
class TestPipelineContext:
    ...
    def test_mutable_state(self, context):
...
class TestIPipelineStep:
    @pytest.mark.asyncio
    async def test_custom_step_executes(self, context):
    ...
    @pytest.mark.asyncio
    async def test_custom_step_failure(self, context):
...
class TestTrainingPipeline:
    @pytest.mark.asyncio
    async def test_pipeline_executes_all_steps(self, context):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_skips_after_error(self, context):
    ...
    @pytest.mark.asyncio
    async def test_add_step_chaining(self, context):
        pipeline = TrainingPipeline()
        pipeline.add_step(DummyStep("a")).add_step(DummyStep("b"))
        assert len(pipeline.steps) == 2

    def test_default_pipeline_has_3_steps(self):
    ...
    def test_from_config_skips_unknown(self):
        config = [{"step": "unknown_step"}]
        pipeline = TrainingPipeline.from_config(config)
...
class TestValidateStep:
    @pytest.mark.asyncio
    async def test_validate_passes_threshold(self, context, tmp_path):
        import json
    ...
    @pytest.mark.asyncio
    async def test_validate_fails_threshold(self, context, tmp_path):
        import json
        metrics_path = tmp_path / "metrics.json"
        ...
        result = await step.execute(context)
        assert result.should_deploy is False
        assert "accuracy=0.8" in result.error

    @pytest.mark.asyncio
    async def test_validate_no_metrics_file(self, context):
        step = ValidateStep(config={"min_accuracy": 0.95})
        result = await step.execute(context)
...
class TestRegisterStep:
    @pytest.mark.asyncio
    async def test_register_records_strategy(self, context):
        step = RegisterStep(config={"strategy": "canary"})
        result = await step.execute(context)
        assert result.artifacts["deploy_strategy"] == "canary"

    @pytest.mark.asyncio
    async def test_register_skips_when_not_deployable(self, context):
        context.should_deploy = False
        step = RegisterStep()
        result = await step.execute(context)
